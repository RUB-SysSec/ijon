	       ...
	       )
{
    ...
    if(g_inFailureMode)
	{
      printf("#  don't exec, g_inFailureMode\n");
    ...
    if(result != TPM_RC_SUCCESS){
      printf("#  couldn unmarshal command tag\n");
	    goto Cleanup;
    }
    ...
    result = UINT32_Unmarshal(&commandSize,
			      ...
			      &command.parameterSize);
    if(result != TPM_RC_SUCCESS){
      printf("#  couldn unmarshal command size\n");
	    goto Cleanup;
    }
    ...
    //IJON_CMP(commandSize,requestSize);
    if(commandSize != requestSize || commandSize > MAX_COMMAND_SIZE)
	{
      printf("#  invalid command size %d %d < %d\n", commandSize, requestSize, MAX_COMMAND_SIZE);
    ...
    if(result != TPM_RC_SUCCESS){
      printf("#  couldn't unmarshal command code\n");
	    goto Cleanup;
    }
    printf("#  unmarshaled command code %d\n", command.code);
    ...
    printf("#  can't perform command before startup %d %d\n", command.code, TPM_CC_Startup);
		result = TPM_RC_INITIALIZE;
		goto Cleanup;
  }
	if(TPMIsStarted() && command.code == TPM_CC_Startup){
    printf("#  can't TPM_STARTUP twice\n");
		result = TPM_RC_INITIALIZE;
		goto Cleanup;
	}
    ...
    if(result != TPM_RC_SUCCESS){
      printf("#  can't parse Handle Buffer\n");
	    goto Cleanup;
    }
    ...
    if(command.tag == TPM_ST_SESSIONS)
	{
	    ...
	    result = UINT32_Unmarshal((UINT32 *)&command.authSize,
				      ...
				      &command.parameterSize);
	    if(result != TPM_RC_SUCCESS) {
        printf("#  can't unmarshal session buffer size\n");
        goto Cleanup;
      }
	    ...
	    if(result != TPM_RC_SUCCESS){
        printf("#  couldn't parse SessionBuffer");
		    goto Cleanup;
      }
	}
    else
	{
	    ...
	    if(result != TPM_RC_SUCCESS){
        printf("#  not authorized for commmand\n");
		    goto Cleanup;
      }
	}
    ...
    if(result != TPM_RC_SUCCESS){
	      goto Cleanup;
        printf("#  falied to dispatch command\n");
    }
