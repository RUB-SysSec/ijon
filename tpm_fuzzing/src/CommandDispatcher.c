...
#include "CommandDispatcher.h"
 ...
 Exit:
    ...
    pAssert(command->index
	    < sizeof(s_CommandDataArray) / sizeof(COMMAND_DESCRIPTOR_t *));
    //IJON_INFO(command->index);
    printf("TRY COMMAND %d\n",command->index);
  ...
  // sanity check //  IJON_CMP(command->parameterSize, 0);
  // sanity check //  if(command->parameterSize != 0)
	// sanity check //{
  // sanity check //    printf("#  command->parameterSize(%d) != 0\n", command->parameterSize);
	// sanity check //    result = TPM_RC_SIZE;
	// sanity check //    goto Exit;
	// sanity check //}
 ...
 Exit:
    if(result==TPM_RC_SUCCESS){
      printf("\t[[[[[EXEC_COMMAND %d]]]]]\n", command->index);
			...
			printf("\tcommand_state: %x\n", command_state);
			//IJON_INFO(command_state);
      //ijon_push_state(index);
    }else{printf("[[[[[[FAIL_EXEC_COMMAND]]]]]]\n");exit(0);}
    MemoryIoBufferZero();
    return result;
#endif
}
